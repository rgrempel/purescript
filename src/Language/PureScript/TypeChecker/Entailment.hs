        ...
        go work con'@(Constraint className' tys' conInfo) = WriterT . StateT . (withErrorMessageHint (ErrorSolvingConstraint con') .) . runStateT . runWriterT $ do
            ...
            case solution of
              Solved substs tcd -> do
                ...
                expr <- lift . lift $ mkDictionary tcd args
          ...
          where
            ...
            mkDictionary :: TypeClassDictionaryInScope -> Maybe [Expr] -> m Expr
            mkDictionary (TypeClassDictionaryInScope (Qualified Nothing (Ident "")) _ cn tys _) Nothing = inlineDictionary cn tys
            mkDictionary (TypeClassDictionaryInScope (Qualified Nothing (Ident "")) _ cn tys _) es = internalError ("Inlined dictionary should not have dependencies: " <> show cn <> " " <> show tys <> " " <> show es)
            mkDictionary tcd Nothing = return $ Var (tcdName tcd)
            mkDictionary tcd (Just []) = return $ Var (tcdName tcd)
            mkDictionary tcd (Just dicts) = return $ foldl App (Var (tcdName tcd)) dicts
